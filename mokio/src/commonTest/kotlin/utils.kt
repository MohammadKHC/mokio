...
private val TIME_TOLERANCE = 1.seconds
internal fun assertTimeIn(range: ClosedRange<Instant>, time: Instant) {
    assertContains(
        range.start - TIME_TOLERANCE..range.endInclusive + TIME_TOLERANCE,
        time
    )
}
...
internal val isMacOs by lazy {
    isUnix && Process(listOf("uname"))
        .outputSource.buffer().use(BufferedSource::readUtf8).trim() == "Darwin"
}
