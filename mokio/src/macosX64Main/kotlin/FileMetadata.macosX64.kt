...
internal actual fun systemFileMetadata(path: Path, followLinks: Boolean): FileMetadata = memScoped {
    val stat = alloc<stat64>()
    val result =
        if (followLinks) stat64(path.toString(), stat.ptr)
        else lstat64(path.toString(), stat.ptr)
