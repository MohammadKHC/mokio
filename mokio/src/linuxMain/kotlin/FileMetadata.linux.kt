...
internal actual fun systemFileMetadata(path: Path, followLinks: Boolean): FileMetadata {
    return statx(path, followLinks) ?: stat(path, followLinks)
}

private fun statx(path: Path, followLinks: Boolean): FileMetadata? = memScoped {
...
private fun stat(path: Path, followLinks: Boolean): FileMetadata = memScoped {
    val stat = alloc<stat>()
    val result =
...
private fun statx_timestamp.toInstant() =
    Instant.fromEpochSeconds(tv_sec, tv_nsec.toInt())
